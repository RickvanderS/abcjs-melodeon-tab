...
function FindFirstUsedIndex(push_row) {
	for (let i = 0; i < push_row.length; ++i) {
		if (push_row[i] != "")
			return i;
	}
	return 0;
...
function DiatonicPatterns(plugin) {
	...
	let numstart = plugin.params.numstart;
	if (numstart == null)
		numstart = 1;
	...
	let numalign = plugin.params.numalign;
	if (numalign == null)
		numalign = 1;
		...
		if (this.tuning.length == 3) {
			...
			if (Row3Tuning == "hohner") {
				...
				if      ((Row2Info.Key == "G"                         ) && (Row3Info.Key == "C"                         )) //DGC, rare mostly conversions with D an octave lower than standard DG
				  ...
				  TransposeHalfSteps =  1;
				else if ((Row2Info.Key == "A"                         ) && (Row3Info.Key == "D"                         )) //EAD, exists (Hohner corona)
				  TransposeHalfSteps =  2;
				else if ((Row2Info.Key == "Bb" || Row2Info.Key == "A#") && (Row3Info.Key == "Eb" || Row3Info.Key == "D#")) //FBbEb, exists (Hohner corona)
				  TransposeHalfSteps =  3;
				else if ((Row2Info.Key == "B"                         ) && (Row3Info.Key == "E"                         )) //GbBE
				  TransposeHalfSteps =  4;
				else if ((Row2Info.Key == "C"                         ) && (Row3Info.Key == "F"                         )) //GCF, exists (Hohner corona)
				  TransposeHalfSteps =  5;
				else if ((Row2Info.Key == "Db" || Row2Info.Key == "C#") && (Row3Info.Key == "Gb" || Row3Info.Key == "F#")) //AbDbGb
				  TransposeHalfSteps =  6;
				else if ((Row2Info.Key == "D"                         ) && (Row3Info.Key == "G"                         )) //ADG, exists (Hohner corona)
				  TransposeHalfSteps =  7;
				else if ((Row2Info.Key == "Eb" || Row2Info.Key == "D#") && (Row3Info.Key == "Ab" || Row3Info.Key == "G#")) //BbEbAb, exists (Hohner corona)
				  TransposeHalfSteps =  8;
				else if ((Row2Info.Key == "E"                         ) && (Row3Info.Key == "A"                         )) //BEA
				  TransposeHalfSteps =  9;
				else if ((Row2Info.Key == "F"                         ) && (Row3Info.Key == "Bb" || Row3Info.Key == "A#")) //CFBb, exists
				  TransposeHalfSteps = 10;
				else if ((Row2Info.Key == "Gb" || Row2Info.Key == "F#") && (Row3Info.Key == "B"                         )) //DbGbB
				  TransposeHalfSteps = 11;
				else {
	...
	if (numalign != 2)
		this.aRowButtonStart[2] += numstart;
	...
	for (let h = 0; h < HandPosCount; ++h) {
		...
		for (let Row = 1; Row <= 3; ++Row) {
			for (let i = 0; i < aFinger.length; ++i) {
				...
				if (Row == 1) {
					let Row1Index = h+Finger;
					if (0 <= Row1Index && Row1Index < this.push_row1.length) this.HandPos[h].hard.push((Row1Index + this.aRowButtonStart[0]).toString() + "$");
				}
				else if (Row == 2) {
					let Row2Index = h+Finger;
					if (0 <= Row2Index && Row2Index < this.push_row2.length) this.HandPos[h].hard.push((Row2Index + this.aRowButtonStart[1]).toString() + "'");
					
				}
				else if (Row == 3) {
					let Row3Index = h+Finger;
					if (0 <= Row3Index && Row3Index < this.push_row3.length) this.HandPos[h].hard.push((Row3Index + this.aRowButtonStart[2]).toString() + "\"");
				}
			}
		}
	}
...
DiatonicPatterns.prototype.noteToPushButtonRow1 = function(noteName) {
	let ButtonNumber = noteToButton(noteName, this.push_row1, this.aRowButtonStart[0]);
...
DiatonicPatterns.prototype.noteToPullButtonRow1 = function(noteName) {
	let ButtonNumber = noteToButton(noteName, this.pull_row1, this.aRowButtonStart[0]);
	if (ButtonNumber.length > 0)
		ButtonNumber += "$";
	return ButtonNumber;
}
...
DiatonicPatterns.prototype.noteToPushButtonRow2 = function(noteName) {
	let ButtonNumber = noteToButton(noteName, this.push_row2, this.aRowButtonStart[1]);
	if (ButtonNumber.length > 0)
		ButtonNumber += "'";
	return ButtonNumber;
}
...
DiatonicPatterns.prototype.noteToPullButtonRow2 = function(noteName) {
	let ButtonNumber = noteToButton(noteName, this.pull_row2, this.aRowButtonStart[1]);
	if (ButtonNumber.length > 0)
		ButtonNumber += "'";
	return ButtonNumber;
}
...
DiatonicPatterns.prototype.noteToPushButtonRow3 = function(noteName) {
	let ButtonNumber = noteToButton(noteName, this.push_row3, this.aRowButtonStart[2]);
	if (ButtonNumber.length > 0)
		ButtonNumber += "\"";
	return ButtonNumber;
}
...
DiatonicPatterns.prototype.noteToPullButtonRow3 = function(noteName) {
	let ButtonNumber = noteToButton(noteName, this.pull_row3, this.aRowButtonStart[2]);
	if (ButtonNumber.length > 0)
		ButtonNumber += "\"";
	return ButtonNumber;
}
...
DiatonicPatterns.prototype.notesToNumber = function (notes, graces, chord) {
	...
	let aFriendlyNoteNames = new Array();
	...
	for (var i = 0; notes && i < notes.length; ++i) {
		...
		let FriendlyNoteName = this.NoteNameAddAccidentals(notes[i].name);
		let noteName = this.helper.NoteAccidentalNormalize(FriendlyNoteName);
		...
		if (!this.showall || this.changenoteheads) {
			...
			if (this.Scan) {
				...
				if (Button.length) {
					...
					if (!Hidden) {
						if (Push)
							strPush = this.AppendButton(strPush, Button, FriendlyNoteName);
						else
							strPull = this.AppendButton(strPull, Button, FriendlyNoteName);
					}
	...
	if (this.showall) {
		...
		for (var i = 0; i < aNoteNames.length; ++i) {
			...
			let FriendlyNoteName = aFriendlyNoteNames[i];
			...
			//Set push buttons
			if (AllowPush) {
				if (AllowRow1 && _push1.length) strPush = this.AppendButton(strPush, _push1, FriendlyNoteName);
				if (AllowRow2 && _push2.length) strPush = this.AppendButton(strPush, _push2, FriendlyNoteName);
				if (AllowRow3 && _push3.length) strPush = this.AppendButton(strPush, _push3, FriendlyNoteName);
			}
			
			//Set pull buttons
			if (AllowPull) {
				if (AllowRow1 && _pull1.length) strPull = this.AppendButton(strPull, _pull1, FriendlyNoteName);
				if (AllowRow2 && _pull2.length) strPull = this.AppendButton(strPull, _pull2, FriendlyNoteName);
				if (AllowRow3 && _pull3.length) strPull = this.AppendButton(strPull, _pull3, FriendlyNoteName);
			}
		}
	}
